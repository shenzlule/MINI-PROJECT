...
#define SensorPin 0 // Analog channel for pH meter output
...
void calculatePH(float *voltage, float *phValue, unsigned long avgValue) {
  *voltage = (float)avgValue * 5.0 / 1024;
  *phValue = 3.5 * (*voltage);

  // Clamp pH value between 1 and 14
  if (*phValue < 1.0) {
    *phValue = 1.0;
...
void loop() {
  ...
  if (currentMillis - previousMillis >= samplingInterval) {
    ...
    // Calculate pH value
    calculatePH(&voltage, &phValue, avgValue);
